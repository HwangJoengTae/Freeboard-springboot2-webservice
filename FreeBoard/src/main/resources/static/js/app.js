const main = {
    ...
    resave : function () {
	   ...
	   const data = {
        ...
        // 공백 및 빈 문자열 체크
        if (!data.title || data.title.trim() === "" || !data.content || data.content.trim() === "") {
           console.log("commentWriterID : " + data.boardCheck);
        ...
        if (con_check === true) {
            if (!data.title || data.title.trim() === "" || !data.content || data.content.trim() === "") {
                ...
                $.ajax({
                    ...
                    alert("수정되었습니다.");
                    window.location.href = '/board/read/' + data.id;
                }).fail(function (error) {
                    alert(JSON.stringify(error));
                });
            }
    ...
    commentSave : function () {
        ...
        if (!data.comment || data.comment.trim() === "") {
            ...
            $.ajax({
                type: 'POST',
                url: '/api/board/' + data.boardId + '/comments',
                dataType: 'JSON',
                contentType: 'application/json; charset=utf-8',
                data: JSON.stringify(data)
            }).done(function () {
    ...
    commentUpdate : function (form) {
        const data = {
            ...
            postsId: form.querySelector('#boardId').value,
        ...
        if (con_check === true) {
            $.ajax({
                type: 'PUT',
                url: '/api/board/' + data.boardId + '/comments/' + data.id,
                dataType: 'JSON',
                contentType: 'application/json; charset=utf-8',
                data: JSON.stringify(data)
            }).done(function () {
  ...
  /** 댓글 삭제 */
    commentDelete : function (boardId, commentId, commentWriterId, sessionUserId) {
        ...
        if (commentWriterId !== sessionUserId) {
            ...
            if (con_check === true) {
                $.ajax({
                    type: 'DELETE',
                    url: '/api/board/' + boardId + '/comments/' + commentId,
                    dataType: 'JSON',
                }).done(function () {
                    alert('댓글이 삭제되었습니다.');
                    window.location.reload();
